...
def load_data(path):
    ...
    forex_toy=forex[(forex.order<=10000)]    
    return 
...
def valid_actions(time, USDreserve):
    ...
    if balance.at[time,'USD_net']>USDreserve:
...
def main(data_path, dollarstoinv, USreserve, USDwealth, min_gain, qlearn, alpha):
    ...
    for index, row in forex_toy.iterrows():
        ...
        global AUDUSD_bid, AUDUSD_ask, EURUSD_bid, EURUSD_ask, GBPUSD_bid, GBPUSD_ask, USDJPY_bid, USDJPY_ask, epsilon, actual_action, actions_list
        ...
        print "balance is: ", balance.at[index+1,'USD_net']
