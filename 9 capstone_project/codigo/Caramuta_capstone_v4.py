...
def main(data_path, dollarstoinv, USreserve, USDwealth, min_gain, qlearn, alpha):
    ...
    state_action_hist = pd.DataFrame(columns=['state', 'action', 'price']) #The history of previous states, actions and prices 
    ...
    for index, row in forex_toy.iterrows():
        ...
        if index%200 == 0:
            balance.to_csv(r'U:/Users/dcaramu/Desktop/Machine-Learning-Nano-NEW/9 capstone_project/codigo/balance_nolearn.txt', header=True, index=True, sep=' ')
