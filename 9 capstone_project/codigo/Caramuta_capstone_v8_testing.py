...
def update_reward(alpha, qlearn, minutes_history):
    ...
    if qlearn==1:
        if len(state_action_hist)>=minutes_history:
            ...
            #if action_reward == 'nothing':
                #reward=0
                #if len(state_action_hist[state_action_hist.action=='sellEURUSD'])>0:
                    #if state_action_hist[state_action_hist.action=='sellEURUSD']['price'].iloc[0] > EURUSD_orig:
                        #reward=-1
                #if len(state_action_hist[state_action_hist.action=='sellAUDUSD'])>0:
                    #if state_action_hist[state_action_hist.action=='sellAUDUSD']['price'].iloc[0] > AUDUSD_orig:
                        #reward=-1
                #if len(state_action_hist[state_action_hist.action=='sellGBPUSD'])>0:
                    #if state_action_hist[state_action_hist.action=='sellGBPUSD']['price'].iloc[0] > GBPUSD_orig:
                        #reward=-1
                #if len(state_action_hist[state_action_hist.action=='sellGBPUSD'])==0 or len(state_action_hist[state_action_hist.action=='sellAUDUSD'])==0 or len(state_action_hist[state_action_hist.action=='sellEURUSD'])==0:
                    #if GBPUSD_bid < GBPUSD_orig and AUDUSD_bid < AUDUSD_orig and EURUSD_bid < EURUSD_orig:
                        #reward = 1
                    #if GBPUSD_bid > GBPUSD_orig or AUDUSD_bid > AUDUSD_orig or EURUSD_bid > EURUSD_orig:
                        #reward = -1                                    
...
def test(data_path, save_Q_path, load_Q_path, minutes_history, dollarstoinv, USreserve, USDwealth, min_gain, qlearn, alpha):
    ...
    Q = np.load(load_Q_path).item() #This is the Q function
